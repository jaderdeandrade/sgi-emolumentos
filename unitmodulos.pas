...
uses unDMAcesso,principal,data, dataEmpresa;
...
begin
 ...
 // cdsUsuarios.ApplyUpdates(0);
  ...
  if (Estado = dsInsert) and (cdsUsuariosItens.IsEmpty) then
    GravaItems_E_PreencheTreeview(_principal, trvModulos);
end;
...
begin
  ...
  begin
    cdsUsuarios.FieldByName('USU_ID').AsInteger := AutoInc('GEN_USUARIOS_ID', dm.Connection1);
...
begin
  ...
  dmEmpresa.cdsFuncionario.Active:= true;
end;
